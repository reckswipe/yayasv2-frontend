  ...
  if (submitted) {
    return (
      <div className="max-w-xl mx-auto py-20 text-center">
        ...
        <a href="/" className="px-8 py-3 bg-gold text-void font-syne uppercase tracking-widest text-sm hover:bg-gold-dim transition-colors">
          Volver al Inicio
        </a>
      </div>
    );
          ...
          {(["info", "shipping", "payment"] as Step[]).map((s, i) => (
            <div key={s} className="flex items-center gap-2">
              <div className={cn(
                ...
                step === s ? "bg-gold text-void" : i < ["info", "shipping", "payment"].indexOf(step) ? "bg-green-600 text-white" : "bg-carbon text-ash"
              )}>
                {i < ["info", "shipping", "payment"].indexOf(step) ? <Check size={16} /> : i + 1}
              </div>
              <span className="text-sm uppercase tracking-widest hidden sm:block">{s === "info" ? "Información" : s === "shipping" ? "Envío" : "Pago"}</span>
          ...
          {step === "info" && (
            <div className="bg-obsidian border border-carbon p-6 space-y-4">
              ...
              <input
                ...
                className="w-full px-4 py-3 bg-carbon border border-carbon text-cream placeholder:text-ash focus:border-gold focus:outline-none"
              />
              <input
                type="text"
                required
              ...
              <button
                ...
                className="w-full py-3 bg-gold text-void font-syne uppercase tracking-widest text-sm hover:bg-gold-dim transition-colors"
              >
                Continuar a Envío
              </button>
            </div>
          ...
          {step === "shipping" && (
            <div className="bg-obsidian border border-carbon p-6 space-y-4">
              ...
              <textarea
                ...
                className="w-full px-4 py-3 bg-carbon border border-carbon text-cream placeholder:text-ash focus:border-gold focus:outline-none resize-none h-20"
              />
              <div className="flex items-center gap-4">
                <div className="flex items-center gap-3 flex-1 py-3 px-4 border border-carbon">
          ...
          {step === "payment" && (
            <div className="bg-obsidian border border-carbon p-6 space-y-4">
              ...
              <div className="space-y-3">
                <button type="button" onClick={() => setPayment("card")} className={cn("w-full flex items-center gap-4 p-4 border transition-colors", payment === "card" ? "border-gold bg-gold/5" : "border-carbon hover:border-smoke")}>
                  <CreditCard size={20} className={payment === "card" ? "text-gold" : "text-ash"} />
                  <div className="text-left"><p className="font-medium">Tarjeta de Crédito/Débito</p><p className="text-xs text-ash">Visa, Mastercard, AMEX</p></div>
                  {payment === "card" && <Check size={20} className="ml-auto text-gold" />}
                </button>
                <button type="button" onClick={() => setPayment("oxxo")} className={cn("w-full flex items-center gap-4 p-4 border transition-colors", payment === "oxxo" ? "border-gold bg-gold/5" : "border-carbon hover:border-smoke")}>
                  <Building size={20} className={payment === "oxxo" ? "text-gold" : "text-ash"} />
                  <div className="text-left"><p className="font-medium">OXXO</p><p className="text-xs text-ash">Pago en efectivo en cualquier OXXO</p></div>
                  {payment === "oxxo" && <Check size={20} className="ml-auto text-gold" />}
                </button>
                <button type="button" onClick={() => setPayment("spei")} className={cn("w-full flex items-center gap-4 p-4 border transition-colors", payment === "spei" ? "border-gold bg-gold/5" : "border-carbon hover:border-smoke")}>
                  <Building size={20} className={payment === "spei" ? "text-gold" : "text-ash"} />
                  <div className="text-left"><p className="font-medium">SPEI / Transferencia</p><p className="text-xs text-ash">Transferencia desde cualquier banco MX</p></div>
                  {payment === "spei" && <Check size={20} className="ml-auto text-gold" />}
                </button>
              </div>
              <div className="flex gap-4">
                ...
                <button type="submit" className="flex-1 py-3 bg-gold text-void font-syne uppercase tracking-widest text-sm hover:bg-gold-dim transition-colors">
                  Completar Orden — {formatPrice(subtotal + (freeShipping ? 0 : 99))}
                </button>
              </div>
            </div>
          )}
      ...
      <div className="lg:col-span-1">
        <div className="bg-obsidian border border-carbon p-6 sticky top-8">
          ...
          <div className="space-y-4 mb-6 max-h-64 overflow-y-auto">
            {items.map((item) => (
              <div key={item.id} className="flex gap-3">
                <div className="w-16 h-16 bg-carbon flex-shrink-0">
                  {item.image?.src && <Image src={item.image.src} alt={item.name} width={64} height={64} className="object-cover" />}
                </div>
          ...
          <div className="space-y-3 border-t border-carbon pt-4">
            ...
            <div className="flex justify-between font-bold text-lg border-t border-carbon pt-3"><span>Total</span><span className="text-gold">{formatPrice(subtotal + (freeShipping ? 0 : 99))}</span></div>
          </div>
        </div>
      </div>
    </div>
